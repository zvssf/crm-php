...
try {
    ...
    $stmt_delete_inputs = $pdo->prepare("DELETE FROM `client_input_values` WHERE `client_id` = :client_id AND `relative_id` IS NULL");
        ...
        foreach ($personas as $persona_data) {
            ...
            if (!empty($p_additional_fields)) {
                $stmt_relative_inputs = $pdo->prepare("
                    INSERT INTO `client_input_values` (`client_id`, `relative_id`, `input_id`, `value`) 
                    VALUES (:client_id, :relative_id, :input_id, :value)
                ");
                foreach ($p_additional_fields as $input_id => $value) {
                     ...
                     if (!empty($value) && is_numeric($input_id)) {
                        $stmt_relative_inputs->execute([
                            ':client_id' => $client_id, 
                            ':relative_id' => $current_relative_id, 
                            ':input_id' => $input_id, 
                            ':value' => $value
                        ]);
                    }
